...
if True:
    def li_niigze(i, j):
        ...
        show_img(input['image'][0], inter_dst=2).save(
            f'{img_dir}/P{i1}-{i2}.png')
    ...
    def br_niigze(i, j):
        ...
        if True:
            nii_name = '/home/hynx/regis/zdongzhong/nifti/brain/original/P{}-{}.nii.gz'.format(
                i1, i2)
...
def li2h5():
    ...
    t1 = F.interpolate(t1.unsqueeze(0).unsqueeze(0), size=(
        128, 128, 128), mode='trilinear').squeeze(0).squeeze(0)
    t1_seg = F.interpolate(t1_seg.unsqueeze(0).unsqueeze(0), size=(
        128, 128, 128), mode='nearest').squeeze(0).squeeze(0)
    print(t1.shape, t1_seg.shape)

    img_dir = '/home/hynx/regis/zdongzhong/images/liver/'
    pa(img_dir).mkdir(exist_ok=True)
    show_img(t1, inter_dst=1).save(
        '{}{}_img.png'.format(img_dir, data_name.name[:-7]))
    t1 = F.interpolate(t1.unsqueeze(0).unsqueeze(0), size=(
        128, 128, 128), mode='trilinear').squeeze(0).squeeze(0)
    show_img(t1, inter_dst=5).save(
        '{}{}_img_128.png'.format(img_dir, data_name.name[:-7]))
    ...
    print(dt1.shape, )
    show_img(t1, inter_dst=5).save(
        '{}{}_img_128h5.png'.format(img_dir, data_name.name[:-7]))
...
def br2h5(t1, t1_seg):
    ...
    pa(img_dir).mkdir(exist_ok=True)
    show_img(t1, inter_dst=1).save(
        '{}{}_original_img.png'.format(img_dir, data_name.name[:-7]))
    print('save img to ', '{}{}_original_img.png'.format(
        img_dir, data_name.name[:-7]))
    ...
    print(dt1.shape, )
    show_img(t1, inter_dst=5).save(
        '{}{}_128h5.png'.format(img_dir, data_name.name[:-7]))
    combo_imgs(dt1, dt1_seg, axis=0, idst=5).save(
        '/home/hynx/regis/zdongzhong/images/brain/{}_combo.png'.format(data_name.name[:-7]))

    seg_on_img = draw_seg_on_vol(dt1, dt1_seg.round().long(
    ), inter_dst=5, to_onehot=True, colors=['red', 'green', 'blue', 'yellow'])
    show_img(seg_on_img, norm=False, inter_dst=1).save(
        '/home/hynx/regis/zdongzhong/images/brain/{}.png'.format(data_name.name[:-7]))
    print('save to ',
          '/home/hynx/regis/zdongzhong/images/brain/{}.png'.format(data_name.name[:-7]))
